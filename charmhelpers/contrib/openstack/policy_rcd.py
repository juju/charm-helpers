...
def policy_config_dir():
    """Directory that contains policy-rc config files.

    :returns: Directory that contains policy-rc config files
    :rtype: str
    """
    return '/etc/policy-rc.d'


def policy_deferred_events_dir():
    """Directory that contains deferred events for a given policy.
...
def policy_deferred_events_files():
    ...
    :rtype: List[str]
    """
    return glob.glob('{}/*.deferred'.format(policy_deferred_events_dir()))
...
def read_yaml_file(file_name):
    ...
    :type file_name: str
    :returns: Objects that were represented by yaml.
    :rtype: ANY
    """
...
def read_default_policy_file():
    ...
    if os.path.exists(policy_file):
        policy = read_yaml_file(policy_file)
    return policy
...
def policy_deferred_events():
    ...
    """
    events = []
    for defer_file in policy_deferred_events_files():
        events.append(read_yaml_file(defer_file))
    return events


def clear_deferred_pkg_events(svcs, action):
    ...
    for defer_file in policy_deferred_events_files():
        deferred_event = read_yaml_file(defer_file)
