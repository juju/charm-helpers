...
import charmhelpers.contrib.declarative


class InstallDeclarativeSupportTestCase(unittest.TestCase):

    def setUp(self):
        super(InstallDeclarativeSupportTestCase, self).setUp()

        patcher = mock.patch('charmhelpers.contrib.declarative.subprocess')
...
class UpdateMachineStateTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = mock.patch('charmhelpers.contrib.declarative.'
                             'juju_config_2_grains')
        self.mock_config_2_grains = patcher.start()
        self.addCleanup(patcher.stop)

    def test_calls_local_salt_template(self):
        charmhelpers.contrib.declarative.update_machine_state(
            'states/install.yaml')
...
class JujuConfig2GrainsTestCase(unittest.TestCase):
    def setUp(self):
        ...
        patcher = mock.patch.object(charmhelpers.contrib.declarative,
    ...
    def test_output_without_relation(self):
        ...
        charmhelpers.contrib.declarative.juju_config_2_grains()
